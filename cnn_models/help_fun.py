...
def forward_and_backward(model, batch, idx_batch, epoch, criterion=None,
    ...
    if use_distillation_loss:
        ...
        softmaxFunction, logSoftmaxFunction, KLDivLossFunction  = nn.Softmax(dim=1), nn.LogSoftmax(dim=1), nn.KLDivLoss(size_average=False)
        if USE_CUDA:
            softmaxFunction, logSoftmaxFunction = softmaxFunction.cuda(), logSoftmaxFunction.cuda(),
            KLDivLossFunction = KLDivLossFunction.cuda()

        if index_distillation_loss.size() != torch.Size([0]):
            ...
            loss_masked = weight_teacher_loss * temperature_distillation**2 * KLDivLossFunction(
                ...
                softmaxFunction(outputsTeacher / temperature_distillation)) / index_distillation_loss.size(0)
...
class LearningRateScheduler:
    def __init__(self, initial_learning_rate, learning_rate_type='generic'):
        ...
        if learning_rate_type not in ('generic', 'cifar100', 'imagenet', 'quant_points_cifar100', 'cifar100_dis'):
    ...
    def update_learning_rate(self, epoch, validation_error):
        ...
        if self.learning_rate_type == 'cifar100_dis':
            optim_factor = 0
            if (epoch > 120):
                optim_factor = 3
            elif (epoch > 100):
                ...
                optim_factor = 1
            new_learning_rate = self.initial_learning_rate * math.pow(0.1, optim_factor)
